...
const getItemsFilterModel = async ({
  ...
  let resultCategorias = { rowCount: 0, rows: [] };
  ...
  if (search && search.trim() !== "") {
    filtros.push(
      format(
        "(libro_titulo ILIKE %L OR libro_autor ILIKE %L)",
        `%${search}%`,
        `%${search}%`
      )
    );
  ...
  if (autor) {
    filtros.push(format("libro_autor = %L", autor));
  }

  if (categoria) {
    if (resultCategorias.rowCount === 0) return [];

    const catValue = resultCategorias.rows
      .filter((cat) => cat.libro_id !== null)
      .map((cat) => cat.libro_id);

    filtros.push(format("libro_id = ANY(%L)", `{${catValue}}`));
  }
  ...
  sqlQuery += format(
    " ORDER BY %I_%I %s LIMIT %L OFFSET %L",
    prefijo,
    campo,
    dir,
    limits,
